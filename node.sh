...
#AKASH = DONE
    0)  tput setaf 1; echo 'What would you like your node name to be?'; tput sgr0
        ...
        export SNAP_RPC_1="https://akash-rpc.polkachu.com:443"
        #export SNAP_RPC_2="http://akashsentry01.skynetvalidators.com:26657"
        ...
        sed -i 's/enable = ""/enable = "true"/g' $WORKINGDIRECTORY/.akash/config/config.toml
        sed -i 's/trust-height = ""/trust-height = "'$BLOCK_HEIGHT'"/g' $WORKINGDIRECTORY/.akash/config/config.toml
        sed -i 's/rpc_servers = ""/rpc_servers = '"$SNAP_RPC_1,$SNAP_RPC_1"'/g' $WORKINGDIRECTORY/.akash/config/config.toml
        sed -i 's/trust-hash = ""/trust-hash = '"$TRUST_HASH"'/g' $WORKINGDIRECTORY/.akash/config/config.toml
